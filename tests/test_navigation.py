...
class NavigationTests(unittest.TestCase):
    def test_lua_bounded_block_observation(self):
        lua = shutil.which('lua5.2') or shutil.which('lua') or shutil.which('luajit')
        if not lua:
            self.skipTest('Install Lua to run bounded observation tests')
        result = subprocess.run([lua, 'tests/block_observation_test.lua'], cwd=ROOT,
